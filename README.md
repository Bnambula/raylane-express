...
├── admin.html          ← Admin dashboard (14 modules)
├── operator.html       ← Operator portal (Basic + Premium)
├── book.html           ← Passenger booking flow
├── render.yaml         ← Render deployment config
├── vercel.json         ← Vercel deployment config
├── _redirects          ← Netlify URL routing
├── api-schema.json     ← Full backend API schema for developers
...
A `vercel.json` is already included — no configuration needed.
...
# Follow prompts — deploy to production when asked
```

Or connect via GitHub at [vercel.com](https://vercel.com) → New Project → Import from GitHub → Vercel auto-detects the `vercel.json`.
